...
void add_edge(graph_t G, unsigned int src, unsigned int dest, int weight){
    if(is_empty_graph(G)) return;
    if(src >= G->num_vertices || dest >= G->num_vertices){
        // these vertices doesn't belong to the graph
        return;
    }
    G->adj_list[src] = insert_in_adj_list(G->adj_list[src], weight, dest);
    return;
}

edge_t insert_in_adj_list(edge_t curr, int weight, unsigned int dest){
    if(curr){
        if(curr->dest != dest){
            curr->next = insert_in_adj_list(curr->next, weight, dest);
        }
        // else: the edge already exists
    }
    else{
        curr = make_edge(weight, dest);
    }
    return curr;
}
...
graph_t transpose(graph_t G){
    if(is_empty_graph(G)) return NULL;
...
void logical_remove_vertex(graph_t G, unsigned int v){ // logical remove
    if(!is_empty_graph(G)){
        if(v >= G->num_vertices){
            // this vertex doesn't belong to the graph
            return;
        }
        if(v < G->num_vertices){
            edge_t tmp = NULL;
...
void remove_vertex(graph_t G, unsigned int v){// physical remove
    if(!is_empty_graph(G)){
        if(v >= G->num_vertices){
            // this vertex doesn't belong to the graph
            return;
    ...
    edge_t* old = G->adj_list;
    G->adj_list = (edge_t*) calloc(--G->num_vertices, sizeof(edge_t));
    ...
    for(unsigned int i=0; i<=G->num_vertices; i++){
        if(i == v){
            ...
            G->adj_list[index++] = remove_vertex_helper(old[i], v);
        }
    }
    free(old);
    // "frees the edge_t* not the edge_t"
    return;
}

edge_t remove_vertex_helper(edge_t adj, unsigned int v){
    if(adj != NULL){
        if(adj->dest == v){
            edge_t tail = adj->next;
            free(adj);
            adj = tail;
            if(adj == NULL) return adj;
        }
        if(adj->dest > v) adj->dest--;
        adj->next = remove_vertex_helper(adj->next, v);
    }
    return adj;
}
...
void bfs(graph_t G, unsigned int starting_point, unsigned int end_point){
    if(starting_point >= G->num_vertices || end_point >= G->num_vertices){
        // these vertices doesn't belong to the graph
        return;
    }
...
graph_stack_t ts(graph_t G){
    ...
    if(has_cycle){
        printf("this graph has a cycle, so it haven't a topological sorting\n");
    }
    printf("ot : ");
    print_graph_list_node(s->head);
    free(reach);
