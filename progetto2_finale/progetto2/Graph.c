...
List bfs(Graph G, int source, int target){
    ...
    int targetFound = 0;
    int v;
    while(!isEmptyQueue(Q)  && !targetFound){
...
Graph populateGraph(){
    ...
    addEdge(g, getCityIndexByName(g, "Napoli"), getCityIndexByName(g, "Milano"), 37.00, 198.00);
 ...
 void PrintGraph(Graph G){
    ...
    for(int i=0; i<G->nodes_count; i++){
        ...
        for(List iter = G->adj[i];iter !=NULL;iter = iter->next){
            printf("--> (id: %d, city: %s, piece:%.2lf, distance:%.2lf)",iter->target, G->cityNames[iter->target], iter->peso[PRICE], iter->peso[DISTANCE]);
    }
    puts("");
    }
    return;
